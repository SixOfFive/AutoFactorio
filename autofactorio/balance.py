...
# Items
# ---------------------------------------------------------------------------
RAW_ORES = ["iron_ore", "copper_ore", "coal", "stone", "uranium_ore"]
...
RECIPES = {
    ...
    # PLUTONIUM: reactor fuel for nuclear plants, refined from mined uranium ore (tech-gated,
    # see NUCLEAR_PLANT_TECH). Extremely energy-dense (PLUTONIUM_ENERGY) - a little runs a
    # nuclear plant a long time. Reactor-only: trains never burn it (not in FUEL_ORDER).
    "plutonium":          {"in": {"uranium_ore": 5},                          "out": {"plutonium": 1},          "time": 8.0},
    # buildables
...
DISPLAY_NAME = {
    ...
    "uranium_ore": "Uranium ore", "plutonium": "Plutonium",
    "boiler": "Boiler", "nuclear_plant": "Nuclear plant",
}
...
# Patch default reserves by ore type (finite -> forces expansion). Uranium is rarer and
# smaller (a little refines into a lot of plutonium), the prize the frontier rewards.
...
# built, so a sprawling factory needs a big supply.
POWER_PER_FURNACE = 0.05           # energy/sec a furnace draws to run
POWER_PER_ASSEMBLER = 0.15         # energy/sec an assembler draws to run

# ---------------------------------------------------------------------------
# Power PLANTS: generation is now a real building you scale (not free fuel-burning)
# ---------------------------------------------------------------------------
# Power is GENERATED by plants and CONSUMED by factories; a plant only generates up to its
# capacity and only while it has fuel. Two kinds:
#   * BOILER - the cheap baseline. Burns the CARBON fuel ladder (coal->fusion, coal
#     penalized), the SAME pool trains draw from - so heavy factory load on boilers starves
#     the trains (the energy problem). Small output.
#   * NUCLEAR PLANT - burns PLUTONIUM (reactor-only, never touched by trains) and puts out
#     ORDERS OF MAGNITUDE more per plant. Powering the base off a few nuclear plants frees
#     the entire carbon supply for trains - the fix for the energy problem. Tech-gated.
# Generation is served nuclear-FIRST (cheap, off the train fuel), boilers fill the rest.
POWER_PLANT_GEN = {"boiler": 0.6, "nuclear": 30.0}     # energy/sec CAPACITY per plant (nuclear=50x)
POWER_PLANT_COST = {                                    # raw materials to build one
    # Built from ABUNDANT materials (concrete/stone + iron) on purpose - NOT steel or
    # circuits, which are the perpetual bottlenecks rolling stock already fights over. The
    # real cost of nuclear is the FUEL chain (mine uranium -> refine plutonium), not the
    # building - so the director can actually stand up the "dozen nuclear plants" endgame.
    "boiler":  {"stone": 25, "iron_plate": 12},
    "nuclear": {"stone": 220, "iron_plate": 90},
}
BOILER_FUELS = ["fusion_fuel", "nuclear_fuel", "refined_fuel", "compressed_coal", "coal"]  # carbon, densest first
NUCLEAR_PLANT_FUEL = "plutonium"   # nuclear plants burn ONLY this
PLUTONIUM_ENERGY = 800.0           # building-power energy per plutonium unit (reactor-grade)
NUCLEAR_PLANT_TECH = 55            # tech level that unlocks nuclear plants + plutonium refining
...
HOME_START = {
    ...
    "boilers": 10,         # baseline power plants (0.6 e/s each) - enough to power the
                           # starting factories with headroom; scale up (or go nuclear) as
                           # the factory count grows or power gets capped
}
...
STOCK_TARGETS = {
    ...
    "plutonium": 80,        # refine mined uranium into reactor fuel (once nuclear is unlocked)
}
...
STORAGE_CAP_START = {
    ...
    "iron_ore": 2500, "copper_ore": 2500, "coal": 2500, "stone": 2500, "uranium_ore": 2500,
    # smelted plates
    ...
    "plutonium": 120,       # reactor fuel; dense, so a modest cap is plenty
    # research currency
    "science_pack": 120,
...
STORAGE_CAP_STEP = {
    "iron_ore": 2000, "copper_ore": 2000, "coal": 2000, "stone": 2000, "uranium_ore": 2000,
...
ORE_WEIGHTS = {"iron_ore": 0.40, "copper_ore": 0.23, "coal": 0.19, "stone": 0.13,
               "uranium_ore": 0.05}   # uranium is rare (5%) - reactor fuel worth seeking out
...
def tech_for_level(level: int) -> dict | None:
    ...
    if level < 1 or level > MAX_TECH_LEVEL:
        return None
    if level == NUCLEAR_FUEL_TECH:
        name, desc = "Nuclear Fuel", "refine coal all the way to nuclear fuel"
    elif level == NUCLEAR_PLANT_TECH:
        name, desc = "Nuclear Power", "unlocks NUCLEAR PLANTS + plutonium refining (massive power)"
    elif level == FUSION_FUEL_TECH:
        name, desc = "Fusion Power", "refine fusion fuel (densest carbon fuel)"
    elif level == SPACE_TECH_LEVEL:
